...
class ThreadController extends BaseController
{
    ...
    public function index(Request $request) {
        $recentDiscussions = $this->api('thread.recent')->parameters(['with' => ['category', 'readers', 'posts']])->get();

        $categories = $this->api('category.index')->parameters(['where' => ['category_id' => 0]])->get();

        event(new UserViewingIndex);

        return view('forum::post.index', compact('categories', 'recentDiscussions'));
    }
    ...
    public function indexNew()
    {
        ...
        return view('forum::thread.index-new', compact('categories', 'threads'));
    }
